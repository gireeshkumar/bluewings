...
//TODO - gireesh,  move to an object/namespace
// 전역 변수 선언
...
function resetImageMapper() {
    phase = 0; // 그리드 제어 단계 (0: 시작점, 1: 종료점, 2: 수정)
    cnt = 0; // 맵 객체 수
}
...
function setMeasure() {
    measure = 'drag';
    // ($('#gnb-menu-drag').hasClass('_active')) ? 'drag' : 'click';
}
...
function generateCode() {
    ...
    for (i = 0; i < cnt; i++) {
        ...
        areaList.push({ shape: "rect", coords: { sx: startCoordX, sy: startCoordY, ex: endCoordX, ey: endCoordY }, link: link });
    }
...
// 그리드 위치 제어 함수
function drawGrid(e) {
    var workspacetop = parseInt($("#workspace").css('top').split('px')[0]) /*gireesh added to adjust the workspace start point*/ ;
    ...
    // 그리드 위치 제어
    $activeGridX.css({ top: (e.pageY - $('#gnb').height() - $('#workspace-ruler-x').height() - 1) - workspacetop });
...
function addLinkToMapEl() {
    ...
    // alert($('.grid-box._active').attr('id'));
    ...
    if ($('.grid-box._active').hasClass('_added')) {
        ...
        $("#mapeleindex").val(urlink).trigger('change');
...
function setLinkElementToMap(value) {
    if (value != null && value != undefined) {
        var addIndex = $('.grid-box._active').attr('id').split('-')[2];
        mapEl[addIndex][2] = value;
        mapEl[addIndex][3] = '_self';
